...
if ( ! class_exists( 'YITH_WCAF_Form_Handler' ) ) {
	...
	class YITH_WCAF_Form_Handler {
		...
		public static function become_an_affiliate( $fields = array() ) {
			...
			YITH_WCAF_Affiliate_Factory::make_user_an_affiliate( get_current_user_id(), $fields );
		...
		public static function generate_referral_url( $fields ) {
			...
			$base_host = wp_parse_url( $original_url, PHP_URL_HOST );
			 ...
			 */
			return apply_filters( 'yith_wcaf_link_generator_generated_url', YITH_WCAF()->get_referral_url( $affiliate->get_token(), $original_url ) );
		}
